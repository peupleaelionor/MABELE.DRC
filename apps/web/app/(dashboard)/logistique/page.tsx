  ...
  return (
    <div className="p-4 lg:p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="font-bold text-2xl text-white">🚛 Logistique</h1>
          <p className="text-sm mt-0.5" style={{ color: 'rgba(255,255,255,0.40)' }}>Transport & livraison partout en RDC</p>
        </div>
        ...
        {SERVICE_TYPES.map((s) => {
          ...
          return (
            <button key={s.id} onClick={() => setActiveType(s.id)}
              ...
              style={{
                backgroundColor: on ? 'rgba(8,145,178,0.12)' : '#242424',
                border:          `1.5px solid ${on ? MOD : 'rgba(255,255,255,0.08)'}`,
                boxShadow:       on ? '0 2px 12px rgba(8,145,178,0.20)' : undefined,
              }}>
              <div className="text-2xl mb-2">{s.emoji}</div>
              <p className="text-xs font-semibold" style={{ color: on ? MOD : 'white' }}>{s.label}</p>
      ...
      <div className="rounded-2xl p-5 mb-6"
        ...
        <h2 className="font-semibold text-sm text-white mb-4">📍 Réserver maintenant</h2>
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <input placeholder="Date & heure" readOnly
              ...
              style={{ backgroundColor: '#2A2A2A', border: '1px solid rgba(255,255,255,0.10)' }} />
            <select className="px-3 py-2.5 rounded-xl text-sm focus:outline-none text-white"
                    style={{ backgroundColor: '#2A2A2A', border: '1px solid rgba(255,255,255,0.10)' }}>
          ...
          <button className="w-full py-3 rounded-xl text-sm font-bold text-white transition-all hover:opacity-90"
                  style={{ backgroundColor: MOD, boxShadow: '0 4px 16px rgba(8,145,178,0.25)' }}>
      ...
      <div className="flex items-center justify-between mb-3">
        <h2 className="font-semibold text-sm text-white">Prestataires disponibles</h2>
        <span className="text-xs" style={{ color: 'rgba(255,255,255,0.35)' }}>{PROVIDERS.length} disponibles</span>
      </div>
      <div className="space-y-3 mb-6">
        {PROVIDERS.map((p) => (
          <div key={p.id} className="rounded-xl p-4 flex items-center gap-4 transition-all hover:-translate-y-0.5 cursor-pointer"
               style={{ backgroundColor: '#242424', border: '1px solid rgba(255,255,255,0.06)' }}>
            <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl flex-shrink-0"
                 style={{ backgroundColor: 'rgba(8,145,178,0.12)', border: '1px solid rgba(8,145,178,0.20)' }}>
              {p.logo}
            </div>
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-0.5">
                ...
                <span className="text-xs" style={{ color: 'rgba(255,255,255,0.35)' }}>{p.trips} courses</span>
              </div>
              <p className="text-xs font-medium mt-1" style={{ color: MOD }}>{p.price}</p>
            </div>
            <button className="px-3 py-2 rounded-lg text-xs font-semibold flex-shrink-0 text-white transition-all hover:opacity-90"
      ...
      <h2 className="font-semibold text-sm text-white mb-4">Comment ça marche</h2>
