'use client'
// ─── Messagerie Sécurisée — Dark Premium ──────────────────────────────────────
...
export default function MessagesPage() {
  ...
  return (
    <div className="flex h-[calc(100dvh-56px)] lg:h-[calc(100dvh-56px)]" style={{ overflow: 'hidden' }}>
      ...
      {/* Conversation list */}
      <div className={`flex-shrink-0 flex flex-col ${active ? 'hidden lg:flex' : 'flex'} w-full lg:w-80`}
           style={{ backgroundColor: '#191919', borderRight: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="px-4 py-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <h1 className="font-semibold text-base text-white mb-3">Messagerie Sécurisée</h1>
          <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm"
                  style={{ color: 'rgba(255,255,255,0.35)' }}>🔍</span>
            <input placeholder="Rechercher..."
                   className="w-full pl-8 pr-3 py-2 text-sm rounded-xl text-white focus:outline-none"
        ...
        <div className="flex-1 overflow-y-auto">
          {CONVERSATIONS.map((c) => (
            <button key={c.id} onClick={() => setActive(c.id)}
              ...
              style={{
                backgroundColor: active === c.id ? 'rgba(224,92,26,0.08)' : 'transparent',
                borderBottom: '1px solid rgba(255,255,255,0.04)',
              }}>
              <div className="relative flex-shrink-0">
                <div className="w-11 h-11 rounded-full flex items-center justify-center text-white font-bold"
                     style={{ backgroundColor: ACC }}>{c.avatar}</div>
                {c.online && (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  ...
                  <p className="text-[10px] flex-shrink-0 ml-2" style={{ color: 'rgba(255,255,255,0.30)' }}>{c.time}</p>
                </div>
                <div className="flex items-center gap-1 mt-0.5">
                  {c.verified && <span className="text-[10px] font-semibold" style={{ color: '#22C55E' }}>✓</span>}
                  <p className="text-xs truncate" style={{ color: 'rgba(255,255,255,0.40)' }}>{c.msg}</p>
                </div>
              </div>
              {c.unread > 0 && (
                <span className="w-5 h-5 rounded-full text-white text-[10px] font-bold flex items-center justify-center flex-shrink-0"
                      style={{ backgroundColor: ACC }}>{c.unread}</span>
              )}
            </button>
          ))}
        </div>
      ...
      {/* Chat window */}
      {active && conv ? (
        <div className="flex-1 flex flex-col lg:flex" style={{ backgroundColor: '#1A1A1A' }}>
          <div className="flex items-center gap-3 px-4 h-14 flex-shrink-0"
            ...
            <button className="lg:hidden text-white" onClick={() => setActive(null)}>←</button>
            <div className="w-9 h-9 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0"
                 style={{ backgroundColor: ACC }}>{conv.avatar}</div>
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-sm text-white">{conv.name}</p>
              <p className="text-xs" style={{ color: conv.online ? '#22C55E' : 'rgba(255,255,255,0.35)' }}>
                {conv.online ? '● En ligne' : 'Hors ligne'}
                {conv.verified && ' · ✓ Vérifié'}
              </p>
            </div>
            <button className="w-9 h-9 flex items-center justify-center rounded-xl"
                    style={{ backgroundColor: '#242424', color: 'rgba(255,255,255,0.55)' }}>⋯</button>
          </div>
            ...
            {msgs.map((m, i) => (
              <div key={i} className={`flex ${m.from === 'me' ? 'justify-end' : 'justify-start'}`}>
                <div className="max-w-[75%] px-4 py-2.5 rounded-2xl text-sm"
                     style={{
                       backgroundColor: m.from === 'me' ? ACC : '#242424',
                       color: 'white',
                       borderBottomRightRadius: m.from === 'me' ? '4px' : undefined,
                       borderBottomLeftRadius:  m.from === 'them' ? '4px' : undefined,
                       border: m.from === 'them' ? '1px solid rgba(255,255,255,0.08)' : undefined,
                     }}>
                  <p>{m.text}</p>
                  <p className="text-[10px] mt-1 text-right"
                     style={{ color: 'rgba(255,255,255,0.50)' }}>{m.time}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="px-4 py-3 flex-shrink-0"
            ...
            <div className="flex items-center gap-2">
              <button className="w-9 h-9 flex items-center justify-center rounded-full flex-shrink-0"
                      style={{ backgroundColor: '#242424', color: 'rgba(255,255,255,0.45)' }}>📎</button>
              <input value={text} onChange={(e) => setText(e.target.value)}
                placeholder="Écrire un message..."
                className="flex-1 px-4 py-2.5 rounded-full text-sm text-white focus:outline-none"
                style={{ backgroundColor: '#2A2A2A', border: '1px solid rgba(255,255,255,0.10)' }} />
              <button disabled={!text}
                className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 transition-all"
                style={{
                  backgroundColor: text ? ACC : 'rgba(255,255,255,0.08)',
                  color: text ? 'white' : 'rgba(255,255,255,0.30)',
                }}
                onClick={() => setText('')}>
                ➤
              </button>
        ...
        <div className="hidden lg:flex flex-1 items-center justify-center" style={{ backgroundColor: '#1A1A1A' }}>
